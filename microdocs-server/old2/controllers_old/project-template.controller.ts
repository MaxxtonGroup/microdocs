...
export class ProjectTemplateController {
  ...
  public async createTemplates(@Body({required:true}) template:Template, @Param("title") title:string, @Param( "tag" ) tag: string, @QueryParam( "env", { required: false } ) envName?: string):Promise<Template>{
    ...
    if ( !validationResult.valid ) {
      throw new BadRequestError( <any>validationResult.errors.map( (error:any) => (error as any).stack ) );
    }
  ...
  public async editTemplate(@Body({required:true}) template:Template, @Param("title") title:string, @Param( "tag" ) tag: string, @Param( "templateName" ) templateName: string, @QueryParam( "env", { required: false } ) envName?: string):Promise<Template>{
    let env = await this.getEnv(envName);
    ...
    // Create template
    return await this.templateService.editTemplate(env, template, project);
  }
