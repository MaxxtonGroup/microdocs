...
export class ProjectComponent {
  ...
  private nodes: Subject<ProjectMetadata[]> = new ReplaySubject<ProjectMetadata[]>( 1 );
  ...
  init() {
    this.tag   = this.pathParams[ 'tag' ];
    this.title = this.pathParams[ 'project' ];
    this.color = StringUtil.getColorCodeFromString( this.title );
    if(!this.tag && this.projects){
      let projectMetadata = this.projects.filter(project => project.title === this.title)[0];
      if(projectMetadata){
        this.tag = projectMetadata.latestTag;
      }
    }
    ...
    if ( this.projects ) {
      this.projects.forEach( project => {
        if ( project.title === this.title ) {
          this.tags = Object.keys(project.tags);
