...
export class RestProjectClient extends RestClient implements ProjectClient {
  ...
  public loadProject( @Query( "env" ) env:string, @Path( "title" ) title:string, @Query( "tag" ) tag:string ):Observable<Project> {
    return null;
  }
  ...
  @Map( resp => resp.json() )
  public importProject( @Query( "env" ) env:string, @Body project:Project, @Path( "title" ) name:string, @Path( "tag" ) tag:string ):Observable<Project> {
    return null;
  }
