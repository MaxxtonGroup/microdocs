...
export class DependencyGraphComponent {
  ...
  isShowTags(): boolean {
    var value = window.localStorage.getItem( 'dashboard.showTags' );
    if ( value === 'true' ) {
      return true;
    }
    return false;
  ...
  transformFlatList( projectNode: ProjectNode, projectTree: ProjectTree ) {
    ...
    if ( projectNode.dependencies ) {
      projectNode.dependencies.forEach( dependency => {
        ...
        let newRefNode       = new ProjectNode( item.title + ":" + item.tag );
        newRefNode.color = item.color;
        newRefNode.reference = "#/" + item.title + ":" + item.tag;
        let newDependency    = new DependencyNode( newRefNode, dependency.type, dependency.problems );
        dependencies.push( newDependency );
      } );
    }
    let newProjectNode = new ProjectNode( projectNode.title + ":" + projectNode.tag, projectTree, projectNode.group, projectNode.tag, projectNode.tags, projectNode.problems );
  ...
  transformInheritanceProject( projectNode: ProjectNode, removeNodes: string[] ): ProjectNode {
    var newNode = new ProjectNode( projectNode.title, undefined, projectNode.group, projectNode.tag, projectNode.tags, projectNode.problems );
